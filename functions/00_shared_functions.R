...
check_dir <- function(directory, create = FALSE, verbose = FALSE) {
  ...
  if (!require(logger)) stop(add_ts("Package logger required but not installed"))
  ...
  for (d in directory) {
    ...
    if (!file.exists(d)) {
      ...
      if (create == FALSE) {
        ...
        stop(log_error("Directory", d, "is required but does not exist.")$default$message)
...
clean_string <- function(x, 
  ...
  if(!require(logger)) stop(add_ts("Package logger is required but not installed"))
  tryCatch({
  ...
  }, error = \(e) stop(log_error("Error using clean_string():\n* {e}")$default$message)
...
trim_faster <- function(x, out = "raster"){
  ...
  if(!require(logger)) stop(add_ts("Missing required package logger"))
...
parse_filename <- function(files) {
  ...
  if(!require(logger)) stop(add_ts("Package logger is required"))
  tryCatch({
